...
class RedisWebsocketDispatcher:
    ...
    def __init__(self, rooms: list[str]):
        self.rooms = rooms
        self.active_connections = {room: [] for room in rooms}
    ...
    def get_nb_connected_users(self, room: str) -> int:
        return self.redis_connection.get(get_redis_nb_users_key(room))
    ...
    def clear_stored_messages(self) -> None:
        """
        Clears the stored messages, for all rooms.
        """
        for room in self.rooms:
            self.redis_connection.delete(get_redis_last_messages_key(room))
